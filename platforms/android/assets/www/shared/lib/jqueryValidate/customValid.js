...
(function($) {
	$.extend($.fn, {
		...
		// ng-js 验证单个控件。
		validRules : function(value,validRule,element,refVal){
			...
			var isExpValid = json.expvalid;
			// 非必填的字段且值为空 那么直接返回成功。
			if ((isRequired == false || isRequired == undefined) && value == "" && isExpValid===undefined)
				return true;
		...
		_validRules :function(conf){
			...
			if ($.type(validValue)  === "boolean")
				_valid = (!rule.rule(value,validValue,element,refVal) && validValue == true) ? false:true;
			else
				_valid = rule.rule(value, validValue,element,refVal);

			//控制校验样式
			if(_valid){
				$(element).removeClass("form_validate");
			}else{
				$(element).addClass("form_validate");
			}


			//错误 提示， qtip 会有
      
  // var qtipApi = $(element).qtip('api');//原有的
      
 //移动端改写的start
  var $nextNode = $(element).next();
  var qtipApi = $($nextNode).qtip('api');
  //移动端改写的end
  if (!_valid){ //验证不通过返回消息
    ...
    if(!qtipApi){
      ...
      if ($next.hasClass && $next.hasClass("error-red")) {
        $next.remove();
      }
      var defaultSetting = {
        aaa : $(element).after('<span class="error-red qtip-default qtip-shadow qtip-red" style="color: red">'+errMsg+'</span>')
  ...
  //置为不可用，隐藏 我
  if(!errMsg){
    if ($nextNode.hasClass && $nextNode.hasClass("error-red")) {
      $nextNode.remove();
    }
